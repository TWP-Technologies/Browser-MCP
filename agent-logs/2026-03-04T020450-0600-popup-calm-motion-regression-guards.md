# Session Log: Popup Calm Motion + Regression Guards

## Scope
Applied a calm-motion profile to popup UI interactions and polling feedback, then added tests to prevent motion timing regressions.
...
- local-mcp-bun/tests/unit/popup_motion_policy.test.ts (new)
- local-mcp-bun/tests/e2e/extension_roundtrip.test.ts

## Verification Run
- `bun run check:popup-ui`
- `bun run build:popup-ui`
- `bun test tests/unit/popup_view_model.test.ts tests/unit/popup_motion_policy.test.ts`
