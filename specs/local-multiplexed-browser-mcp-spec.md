...
- Configurable stale-session cleanup with service-authoritative browser-resource reap, queued-waiter cancellation, and recoverable live MCP transports after idle timeout.
...
- Stale-session cleanup -> Unit (stale selection + waiter cancellation), Integration (soft reap + expired reaped registry close), Regression (post-cleanup transport can recover).
...
- `tool_router`: validates tool input, applies auth, dispatches to domain services, returns structured output/errors, owns cleanup policy, and performs browser-resource reap plus expired reaped registry close.
...
- In daemon/proxy mode, stale-session cleanup MUST keep initialized live ingress sockets open and recoverable, including after their resource-reaped registry entry is removed; direct mode MUST likewise keep stdio alive. Unbound idle ingress sockets MAY be closed only when they lack recoverable initialized MCP state, and unrevived resource-reaped session entries MAY be closed after one additional timeout window.
