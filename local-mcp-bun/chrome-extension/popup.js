...
function format_meridiem_hour(hours_24) {
  const meridiem = hours_24 >= 12 ? "PM" : "AM";
  const hour_12 = hours_24 % 12 === 0 ? 12 : hours_24 % 12;
  return { hour_12, meridiem };
}
function is_same_local_day(left, right) {
  return left.getFullYear() === right.getFullYear() && left.getMonth() === right.getMonth() && left.getDate() === right.getDate();
}
function resolve_toggle_reference_enabled(state) {
...
function compute_next_toggle_target_enabled(state) {
  return !resolve_toggle_reference_enabled(state);
}
function format_snapshot_badge_timestamp(iso_timestamp, now_ms = Date.now()) {
...
var live_refresh_timer = null;
var live_refresh_debounce_ms = 150;
var interaction_guard_window_ms = 350;
var interaction_guard_until_ms = 0;
var interaction_guard_flush_timer = null;
var waiting_countdown_timer = null;
var poll_pulse_timer = null;
var poll_pulse_active = false;
var last_seen_poll_attempt_serial = null;
var last_poll_pulse_started_at_ms = 0;
var poll_pulse_duration_ms = 900;
var poll_pulse_min_gap_ms = 600;
var bridge_url_copy_state = "idle";
var bridge_url_copy_reset_timer = null;
...
function describe_bridge_state(mode) {
  if (mode === "online") {
    return "Online";
  }
  if (mode === "paused") {
...
function bridge_state_class(mode) {
  if (mode === "online") {
...
function describe_poll_schedule(next_reconnect_attempt_at_ms) {
...
function flush_pending_live_refresh_if_ready() {
  ...
  refresh_state(false, false);
}
function schedule_interaction_guard_flush() {
...
function sync_poll_indicator_dom() {
...
function update_poll_status_label_dom() {
...
function sync_waiting_countdown_timer() {
  const mode = resolve_bridge_ui_mode(ui_state?.extension_enabled === true, ui_state?.bridge_connection_state ?? "idle");
...
function sync_waiting_ui_effects() {
  update_poll_status_label_dom();
  sync_poll_indicator_dom();
  sync_waiting_countdown_timer();
}
function resolve_bridge_url_copy_status_label() {
...
function sync_bridge_url_copy_dom() {
...
function clear_bridge_url_copy_reset_timer() {
...
function set_bridge_url_copy_state(next_state) {
...
function fallback_copy_text_to_clipboard(value) {
  try {
...
async function write_text_to_clipboard(value) {
...
async function copy_bridge_url() {
...
function maybe_trigger_poll_pulse(next_state) {
...
async function send_ui_message(type, payload = {}) {
...
async function run_action(callback) {
...
function summarize_close_all_failures(payload) {
...
async function apply_extension_enabled(target_enabled) {
  await send_ui_message("ui_set_enabled", {
    enabled: target_enabled
  });
  await refresh_state(false, false);
}
async function run_disable_with_close_all_sessions() {
  ...
  try {
    try {
      const close_all_result = await send_ui_message("ui_close_all_sessions");
...
function resolve_toggle_next_target_enabled() {
...
function queue_toggle_target_enabled(target_enabled) {
...
async function drain_toggle_queue() {
...
function read_port_input_value() {
...
function render_locked_rows(locks, tab_by_id, disable_all) {
  if (locks.length === 0) {
...
function render_session_rows(sessions, disable_all, stale_session_timeout_minutes, now_ms) {
...
function render() {
  ...
  const bridge_connection_state = ui_state?.bridge_connection_state ?? "idle";
    ...
    <div class="surface">
      ...
      <div class="modal-backdrop ${disable_modal_open ? "" : "modal-backdrop--hidden"}" data-testid="disable-modal-backdrop">
        <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="disable-modal-title">
          ...
          <p>
            ${escape_html(`You have ${active_session_count} active session${active_session_count === 1 ? "" : "s"}. You can disable only, or close all sessions first and then disable.`)}
          </p>
          <div class="modal-actions">
...
function register_event_listeners() {
  ...
  app_root.addEventListener("pointerdown", (event) => {
  ...
  app_root.addEventListener("click", (event) => {
    ...
    if (action === "save-port") {
      ...
      run_action(async () => {
        await send_ui_message("ui_set_port", {
          port: next_port
        });
      });
      return;
